...
import rs.fon.places.common.logger.Log;
import rs.fon.places.R;
 ...
 */
public class PlaceLinear extends LinearLayout {
    ...
    private LinearAnimator mAnimators;
    ...
    public PlaceLinear(Context context) {
        super(context);
        initialize(null, 0);
    }

    public PlaceLinear(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize(attrs, 0);
    }

    @SuppressLint("NewApi")
    public PlaceLinear(Context context, AttributeSet attrs, int defStyle) {
    ...
    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public void setCardStreamAnimator( LinearAnimator animators ){

        if( animators == null )
            mAnimators = new LinearAnimator.EmptyAnimator();
        else
